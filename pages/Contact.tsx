...
const CopyableInfo = ({ label, value, icon }: { label: string; value: string; icon: string }) => {
  ...
  return (
    <div 
      className="flex items-center space-x-6 group p-4 -ml-4 rounded-3xl transition-all"
    >
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all border ${
        ...
        <Icon name={copied ? 'check-circle' : icon} className="w-7 h-7" />
      </div>
      <div className="flex-1">
        <h4 className="font-black text-xs uppercase tracking-widest text-primary mb-1">{label}</h4>
        <div className="flex items-center justify-between">
          <p className="text-black dark:text-white font-bold text-lg">{value}</p>
          <button 
            onClick={handleCopy}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="grid lg:grid-cols-2 gap-20">
        <div>
          <h1 className="text-5xl sm:text-6xl font-black text-black dark:text-white mb-8 leading-[0.9] tracking-tighter uppercase">Let's build <br /> your <span className="text-primary">Website.</span></h1>
          <p className="text-xl text-black dark:text-gray-400 mb-12 leading-relaxed font-bold">
          ...
          <div className="p-10 bg-black/5 dark:bg-white/5 rounded-[40px] border border-black/10 dark:border-white/5 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary/10 blur-3xl rounded-full"></div>
            <div className="flex items-center space-x-3 mb-6 relative z-10">
               <div className="w-2.5 h-2.5 bg-primary rounded-full animate-pulse"></div>
               <span className="text-[10px] font-black text-black dark:text-gray-500 uppercase tracking-[0.3em]">AI Assistant Online</span>
            </div>
            <h4 className="text-2xl font-black text-black dark:text-white mb-6 tracking-tight">Quick AI Help</h4>
            <p className="text-sm text-black/70 dark:text-gray-400 mb-8 font-bold leading-relaxed">
        ...
        <div className="bg-white dark:bg-[#0a0a0a] p-10 md:p-16 rounded-[48px] shadow-2xl border border-black/5 dark:border-white/5 relative overflow-hidden flex flex-col justify-center min-h-[600px]">
          {status === 'success' ? (
